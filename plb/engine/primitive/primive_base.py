...
class Primitive:
    ...
    def __init__(self, cfg=None, dim=3, max_timesteps=1024, dtype=ti.f32, **kwargs):
        ...
        # self.rand_num = ti.field(dtype, shape=(self.num_rand_points,))
        self.rand_points = ti.Vector.field(3, dtype, shape=(max_timesteps,self.num_rand_points))        
    @ti.func
    ...
    @ti.func
    def get_collision_idx(self, f, grid_pos):
        min_dist = 0.
        idx = -1
        for i in range(grid_pos.shape[1]):
            dist = self.sdf(f, grid_pos[f, i])
    ...
    @ti.kernel
    def set_position_grad_kernel(self, s: ti.i32, grad: ti.ext_arr()):
        for j in ti.static(range(3)):
            self.position.grad[s][j] = grad[j]
    
    @ti.kernel
    def set_rotation_grad_kernel(self, s: ti.i32, grad: ti.ext_arr()):
        for j in ti.static(range(4)):
            self.rotation.grad[s][j] = grad[j]
