...
class MPMSimulator:
    def __init__(self, cfg, primitives=()):
        ...
        # Number of particles for collision check
        self.num_particle_collision = 300
        self.rand_num = ti.Vector.field(dim, dtype=dtype,
                                        shape=(self.num_particle_collision,))  # Generating random particles from [-1, 1] using fixed random seed
        self.rand_points = ti.Vector.field(dim, dtype=dtype,
                                           shape=(max_steps, self.num_particle_collision))
        self.collision_idx = ti.field(ti.i32, shape=(max_steps,))
    ...
    def initialize(self):
        ...
        rand_num = np.random.uniform(-1.5, 1.5, (
            self.num_particle_collision, self.dim))  # Larger random numbers to encourage points to be on the corners and edges
    ...
    def set_surface_points(self, s: ti.int32):  # TODO may have issues with multiple collision due to the global data access rule
        for i in ti.static(range(self.n_primitive)):
            for j in ti.static(range(self.n_primitive)):
                if ti.static(self.primitives[i].cfg.collision_group[j] > 0):
                    for k in range(self.num_particle_collision):
                        rand_point = self.rand_num[k] * self.primitives[j].size[None]  # TODO Only works for box for now
                        self.rand_points[s + 1, k] = self.primitives[j].get_surface_pos(s + 1, rand_point)
    ...
    def set_collision_idx(self, s: ti.int32):
        for i in ti.static(range(self.n_primitive)):
            for j in ti.static(range(self.n_primitive)):
                if ti.static(self.primitives[i].cfg.collision_group[j] > 0):
                    self.collision_idx[s + 1] = self.primitives[i].get_collision_idx(s + 1, self.rand_points)

    @ti.kernel
    def apply_collision_projection(self, s: ti.int32):
        for i in ti.static(range(self.n_primitive)):
            for j in ti.static(range(self.n_primitive)):
                if ti.static(self.primitives[i].cfg.collision_group[j] > 0) and self.collision_idx[s+1]!=-1:
                    self.primitives[i].collision_projection(s + 1, self.rand_points[s + 1, self.collision_idx[s+1]])

    @ti.complex_kernel
    def substep(self, s, state=None):
        ...
        for i in range(self.n_primitive):
            if state is None:
                ...
                ## (instead set s state then s+1 state)
                self.primitives[i].set_state(s, state[i])
                self.primitives[i].set_state(s+1, state[i])
            self.primitives[i].sample_points_on_surface(s)
    ...
    def substep_grad(self, s, clear=False):
        self.clear_grid()
        ...
        for i in range(self.n_primitive - 1, -1, -1):
            ...
            ## clear zero grad here!
            else:
                self.clear_primitive_grad(s)
        self.p2g.grad(s)
        self.svd_grad()
    ...
    def get_state(self, f):
        x = np.zeros((self.n_particles, self.dim), dtype=np.float64)  # Pos
        v = np.zeros((self.n_particles, self.dim), dtype=np.float64)  # Vel
        F = np.zeros((self.n_particles, self.dim, self.dim), dtype=np.float64)  # Deformation grad
    ...
    def step(self, is_copy, action=None, state=None):
        ...
        for s in range(start, self.cur):
            self.substep(s, state=state)
        if is_copy:
            self.copyframe(self.cur, 0)  # copy to the first frame for simulation
            self.cur = 0

    def clear_primitive_grad(self, s):
        for i in range(self.n_primitive):
            self.primitives[i].set_position_grad_kernel(s, np.zeros((3)))
            self.primitives[i].set_rotation_grad_kernel(s, np.zeros((4)))
    ...
    def get_torch_x(self, f, device):
        x = torch.zeros(size=(self.n_particles, self.dim), device=device)
