...
class GradModel:
    def __init__(self, env: TaichiEnv, softness=666., init_sampler=None, output_grid=(), return_dist=False):
        ...
        if self.return_dist:
            ...
            for i in self.primitives:
                ...
                if i.state_dim == 8:
                    self.dists += [ti.field(dtype=self.sim.dtype, shape=(self.sim.n_particles,), needs_grad=True)]
                self.dists += [ti.field(dtype=self.sim.dtype, shape=(self.sim.n_particles,), needs_grad=True)]
    ...
    def forward_step(self, s, a, state=None):
        ...
        if state is not None:
            for i in range(s * self.substeps, (s + 1) * self.substeps):
                self.sim.substep(i, state=state)
        else:
    ...
    def backward_step(self, s):
        for i in range((s + 1) * self.substeps - 1, s * self.substeps - 1, -1):
            flag = s == 50 ## TODO: remove later
            self.sim.substep_grad(i, clear=flag)
        for i in self.primitives:
            if i.action_dim > 0:
    ...
    def forward(self):
        if self._forward_func is None:
            class forward(Function):
                @staticmethod
                def forward(ctx, s, a, state, *past_obs):
                    ctx.save_for_backward(torch.tensor([s]), *[torch.zeros_like(i) for i in past_obs])
                    self.forward_step(s, a, state=state)  # the model calculate one step forward
                ...
                def backward(ctx, *obs_grad):
                    ...
                    return (None, actor_grad.reshape(-1), None) + tmp[1:]
